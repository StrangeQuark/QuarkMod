...
public class GrapplingHookEntity extends PersistentProjectileEntity implements FlyingItemEntity {
    ...
    private static final double SLACK_PER_TICK = 0.25D;
    ...
    private ItemStack sourceHookStack = ItemStack.EMPTY;
    private boolean sourceHookDamaged;
    ...
    public void setSourceHookStack(ItemStack sourceHookStack) {
        this.sourceHookStack = ModItems.isGrapplingHook(sourceHookStack) ? sourceHookStack : ItemStack.EMPTY;
    }
    ...
    public void release(boolean returnItem) {
        if (!this.getWorld().isClient && returnItem) {
            this.damageSourceHook();
            if (this.shouldReturnItem) {
                this.returnHookItem();
            }
        }
        this.discard();
    ...
    private void applyTether() {
        ...
        boolean givingSlack = player.isSneaking();
        if (givingSlack) {
            this.giveSlack();
            if (this.reelHolding) {
                this.reelHolding = false;
        ...
        if (this.isReeling()) {
            ...
            if (stretch > 0.0D) {
                if (givingSlack) {
                    velocity = this.applySlackTether(velocity, direction, stretch);
                } else {
                    double radialSpeed = velocity.dotProduct(direction);
                    if (radialSpeed < 0.0D) {
                        velocity = velocity.subtract(direction.multiply(radialSpeed));
    ...
    private void settleAtAnchor(PlayerEntity player, Vec3d velocity) {
        ...
        if (this.reelHolding) {
            this.ropeLength = Math.min(this.ropeLength, REEL_HOLD_RADIUS);
        }
    ...
    private void giveSlack() {
        this.ropeLength = Math.min(this.maxRange, this.ropeLength + SLACK_PER_TICK);
    }

    private Vec3d applySlackTether(Vec3d velocity, Vec3d direction, double stretch) {
        double radialSpeed = velocity.dotProduct(direction);
        double maxOutwardSpeed = -SLACK_PER_TICK;
        if (radialSpeed < maxOutwardSpeed) {
            velocity = velocity.add(direction.multiply(maxOutwardSpeed - radialSpeed));
        }

        double excessStretch = stretch - SLACK_PER_TICK;
        if (excessStretch > 0.0D) {
            velocity = velocity.add(direction.multiply(Math.min(excessStretch * ROPE_CORRECTION, MAX_ROPE_CORRECTION)));
        }
    ...
    private void damageSourceHook() {
        if (this.sourceHookDamaged) {
            return;
        }

        this.sourceHookDamaged = true;
        Entity owner = this.getOwner();
        if (!(owner instanceof ServerPlayerEntity player)) {
            return;
        }

        ItemStack stack = this.findSourceHookStack(player);
        if (stack.isEmpty()) {
            return;
        }

        Hand heldHand = this.getHeldHand(player, stack);
        stack.damage(1, player.getWorld(), player, item -> {
            if (heldHand != null) {
                player.sendEquipmentBreakStatus(item, LivingEntity.getSlotForHand(heldHand));
            }
        });
        player.getInventory().markDirty();
    }

    private ItemStack findSourceHookStack(PlayerEntity player) {
        ItemStack byReference = this.findHookStack(player, stack -> stack == this.sourceHookStack);
        if (!byReference.isEmpty()) {
            return byReference;
        }

        ItemStack controllerStack = player.getStackInHand(this.controllerHand);
        if (this.matchesLaunchedHook(controllerStack)) {
            return controllerStack;
        }

        return this.findHookStack(player, this::matchesLaunchedHook);
    }

    private ItemStack findHookStack(PlayerEntity player, Predicate<ItemStack> predicate) {
    ...
    private boolean matchesLaunchedHook(ItemStack stack) {
        return ModItems.isGrapplingHook(stack) && ItemStack.areItemsAndComponentsEqual(stack, this.getStack());
    }

    private Hand getHeldHand(PlayerEntity player, ItemStack stack) {
